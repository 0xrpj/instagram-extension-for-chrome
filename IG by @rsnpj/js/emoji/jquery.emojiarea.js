...
(function($, window, document) {
  ...
  */
  ...
  var EmojiArea_WYSIWYG = function($textarea, id, options) {
	...
	{
	  // if((e.type == 'keyup') && (e.keyCode == 13) && (e.ctrlKey == true))
	  if(e.type == 'keyup')
	  {
		chrome.storage.local.get('idmExtOption', function(lsItem)
		{
			...
			{	
				// console.log(lsItem);
				var options = JSON.parse(lsItem.idmExtOption);
				if((options.sendShortcut == 'ctrlenter') && ((e.keyCode == 13) && (e.ctrlKey == true)))
				{
					// console.log('Отправляем по Ctrl+Enter');
					triggerSendMessage('ctrlenter');
				}
				if((options.sendShortcut == 'enter') && ((e.keyCode == 13) && (e.ctrlKey == false)))
				{
					// console.log('Отправляем по Enter');
					triggerSendMessage('enter');
				}
			}
			// return false;					 	
		});				
